...
function Letter({ char, origin, index, onComplete }) {
  const [phase, setPhase] = useState("hidden"); // hidden → drifting → wobble → settled
...
export default function GossipLoadingScreen({ onFinished }) {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,600;1,300&family=Cinzel:wght@700&display=swap');
      ...
      >
        ...
        <div
          style={{
            ...
            color: "#ede8ff",
            textShadow:
              "0 0 60px rgba(139, 92, 246, 0.5), 0 0 120px rgba(99, 60, 200, 0.25)",
            userSelect: "none",
          }}
        >
          {LETTERS.map((char, i) => (
